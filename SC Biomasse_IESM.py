...
book = xlrd.open_workbook(os.path.join("Database_IESM_S100_P02_T12.xlsx"))
...
x_MB = {}
for i in supplier:
    contract[i] = m.addVar(vtype=GRB.BINARY,obj=fixed_cost[i], name='contract %s' % (i))
...
#for t in period:
#    for h in refinery:    
#        m.addConstr( == demand[h, t] - demandS[h, t])
...
Purchase_cost=sum(flow[i, j, t]*costA_I[i] for t in period for i in supplier for i, j in arcs.select(i,'*'))    + sum(x_MB[l, b, t]*costA_M[l] for l in market for b in refinery for t in period)

print('total cost= %4.2f'% m.objVal)
print('number of supplier=%4.2f'% sum(contract[i] for i in supplier))
...
labels = 'pretraitement cost',  'holding cost','transport cost','purchase cost','backorder cost','Biofuel production cost'
sizes = [Pretraitement_cost,holding_cost,transport_cost,Purchase_cost, Backorder_cost, Biofuel_production_cost]
...
for h in pretraitement:
    performance = [inventory_bio[h,t] for t in period]
